...
def gen_data_sparse():
    ...
    if os.path.exists(file_path) and reuse==1:
        ...
        x_data = sparse.coo_matrix((data,(rows,cols)), shape=(valid_info_count, chi_size))
        print(x_data.shape)
        return x_data, out_list
...
def train(input, output):
    ...
    train_ratio = 0.9
    train_nums = math.ceil(data_size*train_ratio)

    # 构造训练集 / 测试集

    # train_input = input[:train_nums]
    # train_output = output[:train_nums]
    # test_input = input[train_nums:]
    # test_output = output[train_nums:]
    ...
    # model = AdaBoostClassifier(n_estimators=500,base_estimator=DecisionTreeClassifier(max_depth=3))
